...
const TYPE_SPECIFIC_FIELDS: Record<ProxyType, FieldConfig[]> = {
  ...
  vmess: [
    ...
    {
      name: 'ws-opts.headers',
      label: 'ws-opts.headers',
      type: 'text',
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {allFields.map((field) => (
              <div
                key={field.name}
                className={field.type === 'checkbox' ? 'flex items-center gap-2' : 'grid gap-1.5'}
              >
                {field.type === 'checkbox' ? (
                  <>
                    <input
